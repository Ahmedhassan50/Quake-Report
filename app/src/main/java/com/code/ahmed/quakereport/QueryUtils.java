...
public final class QueryUtils {
    ...
    public static ArrayList<word> extractEarthquakes() {
        ...
        try {
            ...
            JSONObject basejsonres=new JSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray earthquekarray=basejsonres.getJSONArray("features");
            for(int i=0;i<earthquekarray.length();i++){
                JSONObject currentearth=earthquekarray.getJSONObject(i);
                JSONObject properties=currentearth.getJSONObject("properties");
                double power=properties.getDouble("mag");
                String location=properties.getString("place");
                long time=properties.getLong("time");
                String url=properties.getString("url");

                word earth=new word(power,location,time,url);
